...
object UdpfsMsg {
    ...
    /** Human-readable name for logging; falls back to the hex value for anything unrecognized. */
    fun name(msgType: Int): String = when (msgType) {
        OPEN_REQ -> "OPEN_REQ"
        CLOSE_REQ -> "CLOSE_REQ"
        READ_REQ -> "READ_REQ"
        WRITE_REQ -> "WRITE_REQ"
        ...
        LSEEK_REQ -> "LSEEK_REQ"
        DREAD_REQ -> "DREAD_REQ"
        GETSTAT_REQ -> "GETSTAT_REQ"
        MKDIR_REQ -> "MKDIR_REQ"
        REMOVE_REQ -> "REMOVE_REQ"
        RMDIR_REQ -> "RMDIR_REQ"
        BREAD_REQ -> "BREAD_REQ"
        BWRITE_REQ -> "BWRITE_REQ"
        else -> "UNKNOWN"
    }
}
